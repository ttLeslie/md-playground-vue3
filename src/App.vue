<script setup lang="ts">
import { onMounted, ref } from "vue";
import { StreamFetchClient } from "./utils/stream/StreamFetchClient";
import { getCompontentTree } from "./utils/markdown/utils";

interface IContent {
  content: string;
  sequenceNumber: number;
  done?: boolean;
}

// const streamFetchApp = new StreamFetchClient<IContent>(
//   {
//     baseUrl: "http://localhost:3001/chat-request/completions",
//     headers: {
//       "Content-Type": "application/json",
//     },
//     overErrorTimer: 60 * 1000, // 流式中间超时时间，单位为毫秒
//   },
//   {
//     onMessage: (_data) => {
//       content.value += _data?.content;
//       // 处理流式消息
//       const { tree, tokens, error } = getCompontentTree(content.value);
//       console.log(tree, tokens, error);
//     },
//     onClose: (_lastData: IContent) => {
//       // 处理关闭时的回调
//     },
//     onServerError: (_lastData: IContent, _error: Error) => {
//       // 处理服务器错误时的回调
//     },
//     onStreamConnectionError: (_lastData: IContent, _error: Error) => {
//       // 处理流式中间超时错误时的回调
//     },
//     onConnectionError: (_lastData: IContent, _error: Error) => {
//       // 处理连接错误时的回调
//     },
//     onParseError: (_lastData: IContent, _error: Error) => {
//       // 处理 JSON解析错误时的回调
//     },
//   }
// );
// // 开始发起请求，下面是具体的参数
// streamFetchApp.sendStreamRequest({
//   question: "你好",
//   model: "Qwen/QwQ-32B",
// });

// # 一级标题
// ## 二级标题
// ### 三级标题
// #### 四级标题
// ##### 五级标题
// ###### 六级标题

// **这是粗体文本**
// __这也是粗体文本__

// *这是斜体文本*
// _这也是斜体文本_

// ***这是粗斜体文本***

// ~~这是带删除线的文本~~

// - 无序列表项1
// - 无序列表项2
//   - 子列表项2.1
//   - 子列表项2.2

// 1. 有序列表项1
// 2. 有序列表项2
//   1. 子列表项2.1
//   2. 子列表项2.2

// [Element-Plus-X](https://element-plus-x.com "Element-Plus-X")

// ![示例图片](https://element-plus-x.com/logo.png "一张示例图")

// >这是一段引用文本
// >
// >> 这是嵌套的引用文本

// ---

// | 姓名 | 年龄 | 职业 |
// | ---- | ---- | ---- |
// | 张三 | 25   | 工程师 |
// | 李四 | 30   | 设计师 |

// ### 行内代码

// 用 \`ElmentPlusX\` 表示 行内块代码用 \`\` 语句

// ### 代码块

// \`\`\`javascript
// const code = "Element-Plus-X";
// \`\`\`

// ### 行内公式
// $e^{i\\pi} + 1 = 0$

// ### 块级公式
// $$
// F(\\omega) = \\int_{-\\infty}^{\\infty} f(t) e^{-i\\omega t} dt
// $$

// ### mermaid 饼状图

// \`\`\`mermaid
// pie
//     "传媒及文化相关" : 35
//     "广告与市场营销" : 8
//     "游戏开发" : 15
//     "影视动画与特效" : 12
//     "互联网产品设计" : 10
//     "VR/AR开发" : 5
//     "其他" : 15
// \`\`\`

// [Baidu](https://www.baidu.com)

const content = ref(`[Baidu](https://www.baidu.com)`);

const { tree, tokens, error } = getCompontentTree(content.value);
console.log(tree, tokens, error);
</script>
<template></template>

<style scoped></style>
